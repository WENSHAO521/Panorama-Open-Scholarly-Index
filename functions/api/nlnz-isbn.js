/**
 * Cloudflare Pages Function — proxy for National Library of New Zealand (Te Puna) SRU API.
 * No API key required; this proxy exists solely to bypass CORS restrictions.
 *
 * Usage: GET /api/nlnz-isbn?isbn=9780908992270
 */
export async function onRequestGet({ request }) {
  ...
  try {
    upstream = await fetch(
      `https://nlnz.alma.exlibrisgroup.com/view/sru/64NZ_NLNZ?${params.toString()}`,
      {
  ...
  if (!upstream.ok) return json({ found: false, error: `NLNZ ${upstream.status}` }, 502)

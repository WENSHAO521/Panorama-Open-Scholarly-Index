/**
 * Cloudflare Pages Function — proxy for Library and Archives Canada (LAC/BAC) SRU API.
 * No API key required; this proxy exists solely to bypass CORS restrictions.
 *
 * Usage: GET /api/lac-isbn?isbn=9780771099755
 */
export async function onRequestGet({ request }) {
  ...
  try {
    upstream = await fetch(
      `https://epe.lac-bac.gc.ca/100/200/sru/lac?${params.toString()}`,
      {
  ...
  if (!upstream.ok) return json({ found: false, error: `LAC ${upstream.status}` }, 502)
  ...
  // LAC uses "Lastname, Firstname" — convert to "Firstname Lastname"
